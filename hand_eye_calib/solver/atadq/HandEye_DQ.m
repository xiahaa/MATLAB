...
%Our quaternions are like this (q1 q2 q3,s )
    ...
    for i=1:n
        ...
        [q,qprime] = getDualQ(A(1:3,1:3),A(1:3,4));               
        Qa(i).q = q;
        Qa(i).qprime = qprime;
        [q,qprime] = getDualQ(B(1:3,1:3),B(1:3,4));                
        Qb(i).q = q;
        Qb(i).qprime = qprime;
    end
    ...
    S = [];
    for i=1:n
        S(:,:,i) = [Qa(i).q(1:3)-Qb(i).q(1:3)   skewm(Qa(i).q(1:3)+Qb(i).q(1:3)) zeros(3,1) zeros(3,3);...
                    Qa(i).qprime(1:3)-Qb(i).qprime(1:3)   skewm(Qa(i).qprime(1:3)+Qb(i).qprime(1:3)) Qa(i).q(1:3)-Qb(i).q(1:3)   skewm(Qa(i).q(1:3)+Qb(i).q(1:3))];                                
    end  
