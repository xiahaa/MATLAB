function hierarchicalBoxplotSimple(x,grp,separatorlines,color, colors)
...
boxplot(x, (grp(:,2)),...
    ...
    'factorseparator',1);%'plotstyle','compact',

if nargin>=5
    if size(colors,1)~=numgrps(1)
...
if nargin>=5
    ...
    cmap = hsv(numgrps(1))';
    cmap=vertcat(cmap,ones(1,numgrps(1))*0.2);
end
colorpatch=repmat(cmap, 1, N/numgrps(1));
% Apply colors
h = findobj(gca,'Tag','Box');
for jj=1:length(h)
   patch(get(h(jj),'XData'),get(h(jj),'YData'),colorpatch(1:3,jj)','FaceAlpha',colorpatch(4,jj));
end
